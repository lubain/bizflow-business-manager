...
const STATUS_ACTIONS: Record<
  ExpenseStatus,
  { status: ExpenseStatus; label: string; icon: any; color: string }[]
> = {
  pending: [
    {
      status: "approved",
  ...
  return (
    <>
      <button
        ...
        onClick={toggle}
        className={cn(
          "flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium transition-opacity hover:opacity-80",
...
export default function ExpensesPage() {
  ...
  const updateStatus = useMutation({
    ...
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({ queryKey: ["expenses"] });
      queryClient.invalidateQueries({ queryKey: ["dashboard"] });
      toast.success(`Dépense ${STATUS_LABELS[vars.status].toLowerCase()}`);
    },
        ...
        <div className="grid grid-cols-3 lg:grid-cols-1 gap-3">
          {[
            ...
            >
              ...
              <p className={cn("text-lg md:text-xl font-semibold", color)}>
                {value}
              </p>
            </div>
          ))}
